...
<!-- 
	내장객체의 영역
		...
		요청들이 모여 하나의 세션을,
		세션이 모여 하나의 어플리케이션을 이룬다.
	
	page 영역 : 동일한 페이지에서만 공유
	request 영역 : 하나의 요청에의해 호출된 페이지 및 포워드 페이지
	session 영역 : 클라이언트가 처음 웹브라우저로 접속한 후 웹브라우저를 닫을때까지
	...
	getAttribute(name)
		영역에 저장된 속성값을 얻어옴
		Object형태로 자동 형변환되어 저장되므로 원래타입으로 형변환후 사용 가능 
...
%>
 	...
 	페이지 영역이 초기화 된다.
 	 -->
 	<a href="PageLocation.jsp">"PageLocation.jsp로 이동</a>
