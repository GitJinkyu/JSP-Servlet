...
public class PageDto {
	...
	int realEnd;		//게시물의 끝 페이지 (끝 페이지로 이동 >>)
	boolean prev,next;	//이전, 다음 버튼(true:보여주기)

	
	//페이지블럭을 생성하기 위해 필요한 요소
	int total;			//총게시물의 수
	Criteria criteria;	//페이지번호,페이지당 게시물수
	...
	public PageDto(int total, Criteria criteria) {
		...
		//페이지 블럭의 끝번호 구하기
		//ex 7 페이지 요청 : 7/10 -> 올림처리 -> * 10 =endNo
		endNo = (int)(Math.ceil(criteria.pageNo/10.0) * 10);
		startNo = endNo - (10-1);
		
		//마지막 게시물이 있는 끝 페이지 번호
		//ex 55/10 = 5 -> 더블타입으로 계산 후 올림처리
		//소수점 계산을 하기 위해서 total에 1.0을 곱해서 소수점으로 변환
